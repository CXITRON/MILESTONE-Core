...
milestone-release local 2.3.5 "Fix MEDIA stream pacing and starvation"
```
...
```bash
cd /tmp && milestone-release taildrop 2.3.5 "Fix MEDIA stream pacing and starvation"
```
...
./tools/make-release.sh 2.3.5 "Fix MEDIA stream pacing and starvation"
```

기본 설명을 사용하려면 `./tools/make-release.sh 2.3.5`만 실행할 수 있습니다. 다른 CLI를 사용해야 할 때는 `ARDUINO_CLI=/경로/arduino-cli`로 지정합니다. 임의로 내보낸 BIN을 받지 않으므로 잘못된 PSRAM·파티션 설정이 릴리스에 섞이지 않습니다.
...
2. 버전과 동일한 태그를 만듭니다. 예: `v2.3.5`
3. Release 제목을 `MILESTONE Core v2.3.5`로 지정합니다.
