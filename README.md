...
./tools/make-release.sh 1.5.1 MILESTONE_Core.ino.bin "기기 세부정보 화면 추가"
```
...
2. 버전과 동일한 태그를 만듭니다. 예: `v1.5.1`
3. Release 제목을 `MILESTONE Core v1.5.1`로 지정합니다.
4. `release/MILESTONE_Core.bin`과 `release/MILESTONE_Core.json`을 첨부합니다.
5. Pre-release가 아닌 최신 정식 Release로 게시합니다.
6. 1.5.1을 USB로 설치한 뒤 이후 버전에서 OTA 업데이트를 검증합니다.
...
업데이트가 실패해도 현재 실행 중인 펌웨어는 그대로 유지됩니다. 다만 새 펌웨어가 검증 후 부팅 초기에 완전히 멈추는 경우 Arduino 기본 부트로더 설정에서는 자동 롤백이 보장되지 않으므로, USB 복구가 필요할 수 있습니다.
