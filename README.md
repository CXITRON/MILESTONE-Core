...
어두운 표지가 거의 검게 뭉개지는 경우를 줄이기 위해 Worker는 원본 평균 밝기가 110 미만일 때만 자동 보정을 적용합니다. 미리 계산한 gamma 0.8 LUT와 최대 32단계 노출 상승을 어두운 정도에 비례해 혼합하고, 밝기 0~6의 완전한 검정과 평균 110 이상의 정상 표지는 그대로 둔 뒤 기존 4×4 Bayer를 적용합니다. 하나의 보정 곡선을 60×60·88×88 출력이 공유하며, 기존 변환 대비 로컬 측정 추가 비용은 두 출력 합계 약 0.03ms입니다. 변환 캐시 키를 `mab1-adaptive-tone-v1`로 분리해 기존 30일 캐시 이미지는 즉시 재생성됩니다.
