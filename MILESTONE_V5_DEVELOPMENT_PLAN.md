...
> 상태: 현재 소스 v5.2.4. 이 문서는 설계 결정과 단계별 구현 이력을
...
> 소스 기준은 5.2.4이며 이후 변경은 소스, 테스트와 릴리스 문서를 함께
> 동기화한다.
...
ZERO가 BLE로 바쁘고 MAIN AP·MEDIA 작업이 없음:
  MAIN이 저장 Wi-Fi로 제한 시간의 인터넷 작업을 맡음
  설정 AP가 열려 있으면 접속자 유무와 무관하게 MAIN 인계 보류

MAIN과 ZERO가 모두 바쁨:
  새 인터넷 작업은 보류하거나 구체적 실패 사유를 표시
  사용자 요청도 현재 BLE 연결이나 AP를 강제로 끊어 처리하지 않음

OTA:
  확인·다운로드는 위 역할 배정 적용
  실제 설치는 기존 물리 확인 뒤 BLE와 선택 기능을 정지하고 무결성 우선

프로필:
  NOW만 BLE 활성화. CORE·MEDIA·설정 AP·안전 상태는 BLE 중지
  주변장치·SD·Flash 설치 소유권은 이동하지 않음
```
...
-> 전체 성공 후 10분 안정화 완료
-> 별도로 서명된 관리자 지정 버전만 SD Stable/Backup 갱신
```
...
### 2026-09-13 — v5.2.3 부하 안정성 및 상태 표시

2026-09-05/06 기록의 자동 Stable 승격과 초기 AP 양보 정책은 당시 구현 이력이다.
현재는 설치 성공과 관리자 Stable 지정을 분리하고, AP 또는 MEDIA 고부하 중에는
MAIN에 새 인터넷 작업을 맡기지 않는다. 버튼 샘플러·TFT/SD 전송 경계·Sync raw
업로드·프로필 BLE 정책·독립 LED·12페이지 기기 정보와 검증 범위는
[v5 작업 보고](v5/WORK_CHECKPOINT_20260913_STABILITY.md)를 따른다.

### 2026-09-13 — v5.2.4 ZERO 링크·Sync 긴급 회귀 수정

5.2.3의 콜백 수신 시각과 이전 루프 시각 비교가 unsigned wrap을 일으켜 정상
ZERO를 stale로 지우는 결함을 재현했다. 콜백 이후 시각을 갱신하며 실제 5초
무응답 검사는 유지한다. OTA 관찰 대기에는 Sync를 허용하고 실제 파일/설치
작업은 차단한다. 첫 raw 요청 거절 시 오류 전달과 변환 전 수신 가능 확인을
추가했다. 상세 검증은 `v5/WORK_CHECKPOINT_20260913_HOTFIX.md`에 기록한다.
