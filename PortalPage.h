...
async function installUpdate(){if(!confirm('새 펌웨어를 설치하고 재부팅하시겠습니까? 설치 중에는 전원을 분리하면 안 됩니다.'))return;setStatus('펌웨어 설치 요청을 전송합니다…','ok');try{const d=await api('/api/update/install',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:form({})});if(d.state==='reconnecting'){setStatus('저장된 Wi-Fi에 다시 연결하고 최신 정보를 확인합니다. 연결 후 설치 버튼을 다시 눌러 주세요.','ok');setTimeout(refresh,1500)}else{setStatus('요청 접수 → 다운로드 → 검증 → 재부팅 순서로 진행합니다. OLED에서 진행률을 확인해 주세요.','ok')}}catch(e){setStatus(e.message,'bad')}}
