...
require 'FIRMWARE_VERSION\[\] = "2\.2\.21"' MILESTONE_Core.ino 'firmware version is not 2.2.21'
...
require 'MILESTONE_NOW_ARTWORK_RUNTIME_V2' CoreArtwork.inc 'NOW artwork runtime marker missing'
require 'NOW_ARTWORK_SERVICE_URL' MILESTONE_Core.ino 'NOW artwork gateway URL is missing'
require 'https://milestone-artwork\.typhoon-individual\.workers\.dev/v1/artwork' MILESTONE_Core.ino 'NOW artwork gateway deployment URL is not configured'
require 'NOW_ART_GATEWAY_ATTEMPTS = 2' CoreArtwork.inc 'NOW artwork gateway retry bound is missing'
require 'nowArtworkReadGatewayJpeg' CoreArtwork.inc 'NOW artwork gateway client is missing'
require 'http\.POST\(body\)' CoreArtwork.inc 'NOW artwork metadata must use a bounded POST body'
require 'if \(useGateway\)' CoreArtwork.inc 'configured artwork gateway must own the hot path'
require 'if \(!useGateway && !success\)' CoreArtwork.inc 'gateway failure must not fan out into direct provider requests'
...
require 'if \(!useGateway && !success\) \{' CoreArtwork.inc 'MusicBrainz query allocations must remain lazy after Apple or gateway success'
