...
require 'FIRMWARE_VERSION\[\] = "2\.2\.24"' MILESTONE_Core.ino 'firmware version is not 2.2.24'
...
require 'MILESTONE_BLE_AMS_RUNTIME_V6' CoreBluetooth.inc 'compiled BLE runtime marker missing'
...
require 'AMS_CONN_INTERVAL_MIN = 24' CoreBluetooth.inc 'AMS connection interval is not coexistence-safe during artwork HTTPS'
require 'AMS_CONN_INTERVAL_MAX = 32' CoreBluetooth.inc 'AMS maximum connection interval is too sparse during artwork HTTPS'
...
require 'MILESTONE_NOW_ARTWORK_RUNTIME_V4' CoreArtwork.inc 'NOW artwork runtime marker missing'
...
require 'esp_coex_preference_set\(ESP_COEX_PREFER_BT\)' CoreArtwork.inc 'artwork TLS must preserve live AMS airtime'
...
require 'nowArtworkWorkerWifiPowerSave = WiFi\.setSleep\(true\)' CoreArtwork.inc 'artwork TLS must create Wi-Fi modem-sleep slots for BLE'
require 'WiFi\.setSleep\(false\)' CoreArtwork.inc 'artwork completion must restore fully awake Wi-Fi'
require 'bluetoothNowPlayingHasLiveConnection\(\).*return true' CoreArtwork.inc 'artwork layouts must keep STA associated during a live iPhone session'
