...
require 'maxFrames=Math\.floor\(maxBytes/FRAME_BYTES\)' StreamPage.h 'preconversion frame ceiling must account for mono/color frame size'
...
require '인코딩' StreamPage.h 'stream UI must expose browser encoder latency'
...
require 'mediaStreamFrameBytes != MilestoneMedia::COLOR_FRAME_BYTES' CorePortal.inc 'color raw requests must drain before a full TFT flush competes with SoftAP receive'
