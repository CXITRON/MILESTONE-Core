...
require 'CONFIG_VERSION = 11' MILESTONE_Core.ino 'config schema is not 11'
...
require 'MEDIA_STREAM_MAX_PUSH_FRAMES = 4' CoreMedia.inc 'bounded color stream push missing'
require 'MEDIA_STREAM_PREBUFFER_FRAMES = 32' CoreMedia.inc 'initial color stream prebuffer missing'
require 'MEDIA_STREAM_REBUFFER_FRAMES = 16' CoreMedia.inc 'color stream rebuffer threshold missing'
...
require 'INITIAL_FILL_FRAMES=32' StreamPage.h 'browser color initial fill missing'
require 'LOW_WATERMARK=16' StreamPage.h 'stream sender low watermark missing'
require 'REFILL_TARGET=72' StreamPage.h 'stream sender refill target missing'
require 'REFILL_FLOOR=48' StreamPage.h 'stream sender refill floor missing'
...
require 'MAX_PUSH_FRAMES=4' StreamPage.h 'browser color raw push bound missing'
...
require 'MEDIA_STREAM_PREBUFFER_FRAMES = 32' CoreMedia.inc 'device must accumulate a color-safe initial queue before playback'
require 'MEDIA_STREAM_REBUFFER_FRAMES = 16' CoreMedia.inc 'device must rebuild color-safe queue depth after an underrun'
...
require 'MEDIA_STREAM_MAX_PUSH_FRAMES = 4' CoreMedia.inc 'device raw push bound must match the color browser batch'
