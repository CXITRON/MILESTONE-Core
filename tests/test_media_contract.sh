...
require 'MEDIA_STREAM_JPEG_MAX_BYTES = 12U \* 1024U' CoreMedia.inc 'live JPEG frame ceiling missing'
require 'MEDIA_STREAM_PREBUFFER_FRAMES = 24' CoreMedia.inc 'monochrome stream prebuffer missing'
require 'MEDIA_STREAM_REBUFFER_FRAMES = 12' CoreMedia.inc 'monochrome stream rebuffer threshold missing'
require 'MEDIA_STREAM_JPEG_PREBUFFER_FRAMES = 48' CoreMedia.inc 'stable color stream prebuffer missing'
require 'MEDIA_STREAM_JPEG_REBUFFER_FRAMES = 24' CoreMedia.inc 'color stream rebuffer threshold missing'
...
require 'MilestoneMedia::decodeFrameSized' CoreMedia.inc 'live monochrome stream must decode MSM frame records before queueing'
require 'MilestoneMedia::parseLiveJpegRecord' CoreMedia.inc 'live color stream must validate JPEG frame records before queueing'
require 'esp_jpeg_decode' CoreMedia.inc 'live color stream must decode JPEG on the ESP32-S3'
require 'loadRgb565Native' CoreMedia.inc 'decoded live JPEG must enter the RGB565 TFT layer'
...
require 'xTaskCreatePinnedToCore\(mediaStreamWsTaskMain' CoreMedia.inc 'WebSocket receiver task creation missing'
require 'nullptr, 2, &mediaStreamWsTaskHandle, 0\)' CoreMedia.inc 'WebSocket receiver must run independently on core 0'
require 'mediaStreamWsIngress' CoreMedia.inc 'WebSocket receiver needs a separate PSRAM ingress buffer'
require '__atomic_store_n\(&mediaStreamWsMessageReady' CoreMedia.inc 'WebSocket producer/consumer handoff must publish complete messages atomically'
require 'processMediaStreamWebSocketIo' CoreMedia.inc 'dedicated task must own socket reads'
require 'STREAM_MODE WebSocket reject:' CoreMedia.inc 'WebSocket protocol rejection needs serial diagnostics'
...
require 'INITIAL_FILL_FRAMES=72' StreamPage.h 'browser color initial fill missing'
...
require 'MAX_WS_IN_FLIGHT=2' StreamPage.h 'WebSocket sender needs a conservative bounded ACK pipeline'
require 'WS_PIPELINE_WARMUP_ACKS=2' StreamPage.h 'WebSocket pipeline must warm up through acknowledged messages'
require 'state\.wsWindow=1' StreamPage.h 'WebSocket pipeline must begin with one message in flight'
require 'Promise\.all\(jobs\)' StreamPage.h 'WebSocket refill batches must be sent concurrently'
require 'estimatedQueue\(\)\+state\.inFlightFrames' StreamPage.h 'unacknowledged WebSocket frames must reserve queue credit'
...
require 'id="buffer_limit"' StreamPage.h 'live preconversion needs a selectable browser memory ceiling'
require '<option value="48" selected>' StreamPage.h 'live preconversion needs an iPhone-safe default ceiling'
require '<option value="96">' StreamPage.h 'live preconversion needs an opt-in expanded ceiling'
require 'state\.storedBytes\+frame\.byteLength>maxBytes' StreamPage.h 'preconversion memory ceiling must use actual encoded bytes'
...
require 'return null' StreamPage.h 'live XOR-RLE encoder must abandon incompressible frames early'
require 'function encodeBatch\(' StreamPage.h 'live encoded frame-record batching missing'
require 'function encodeJpegFrame\(' StreamPage.h 'browser live JPEG encoder missing'
require 'id="jpeg_budget"' StreamPage.h 'color stream needs a selectable JPEG byte budget'
require 'MAX_PUSH_BYTES=32768' StreamPage.h 'JPEG POSTs need a bounded byte ceiling'
require 'RAW 대비' StreamPage.h 'stream UI must expose payload compression ratio'
require '브라우저 묶음' StreamPage.h 'stream UI must expose browser batch-building latency'
...
require 'queue>LOW_WATERMARK' StreamPage.h 'low-watermark refill trigger missing'
require 'pushBurst\(index,REFILL_TARGET\)' StreamPage.h 'watermark refill target missing'
...
require 'MEDIA_STREAM_JPEG_PREBUFFER_FRAMES = 48' CoreMedia.inc 'device must accumulate a stable color queue before playback'
require 'MEDIA_STREAM_JPEG_REBUFFER_FRAMES = 24' CoreMedia.inc 'device must rebuild enough color queue after an underrun'
...
require '!mediaStreamJpegMode && mediaStreamPhase == MediaStreamPhase::PLAYING' CorePortal.inc 'JPEG receive must not be stalled by in-callback display work'
require 'MEDIA_STREAM_WS_PORT = 81' CoreMedia.inc 'dedicated WebSocket stream listener missing'
require 'processMediaStreamWebSocket\(\)' CoreRuntime.inc 'STREAM_MODE must service the persistent socket'
require 'parseLiveWebSocketPushHeader' CoreMedia.inc 'WebSocket stream metadata must be validated from the binary message'
require 'new WebSocket\(streamWsUrl\(\)\)' StreamPage.h 'browser persistent WebSocket sender missing'
require "state.transport='http'" StreamPage.h 'HTTP transport must remain a bounded compatibility fallback'
...
require 'loadRgb332' CoreMedia.inc 'stored RGB332 output path missing'
...
reject 'shouldRotateStoredItem' CoreRuntime.inc 'stored media selection must not use the legacy catalog timer'
require 'Stored MEDIA selection is physical-button only' CoreRuntime.inc 'stored media selection must document the physical-button-only contract'
