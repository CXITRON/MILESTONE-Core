...
require 'MEDIA_STREAM_PREBUFFER_FRAMES = 20' CoreMedia.inc 'initial stream prebuffer must materially use the PSRAM queue'
require 'MEDIA_STREAM_REBUFFER_FRAMES = 12' CoreMedia.inc 'stream rebuffer threshold must rebuild jitter margin'
...
require 'server\.collectHeaders\(headerKeys, 4\)' CorePortal.inc 'raw stream metadata headers must be explicitly collected'
...
require 'X-MILESTONE-Count' CorePortal.inc 'raw stream frame-count header handling missing'
...
require 'INITIAL_FILL_FRAMES=20' StreamPage.h 'browser initial fill must use the PSRAM queue instead of a tiny lead buffer'
require 'LOW_WATERMARK=12' StreamPage.h 'stream sender low watermark missing'
require 'REFILL_TARGET=24' StreamPage.h 'stream sender refill target missing'
require 'REFILL_FLOOR=22' StreamPage.h 'stream sender refill floor missing'
require 'QUEUE_SAFETY_FRAMES=2' StreamPage.h 'stream sender must reserve ACK transit safety frames'
...
require 'MEDIA_STREAM_PREBUFFER_FRAMES = 20' CoreMedia.inc 'device must accumulate a meaningful initial queue before playback'
require 'MEDIA_STREAM_REBUFFER_FRAMES = 12' CoreMedia.inc 'device must rebuild queue depth after an underrun before resuming'
