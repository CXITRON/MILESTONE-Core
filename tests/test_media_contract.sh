...
require 'MEDIA_STREAM_MAX_PUSH_FRAMES = 24' CoreMedia.inc 'bounded raw stream push missing'
...
require 'MAX_PUSH_FRAMES=24' StreamPage.h 'browser raw push bound must amortize connection-close overhead'
require 'PUSH_TIMEOUT_MS=5000' StreamPage.h 'stream POST needs a bounded transport timeout'
require 'new AbortController\(\)' StreamPage.h 'stream POST timeout must abort a stalled fetch'
require 'observedDrainFps\(\)' StreamPage.h 'queue drain must use measured device output FPS'
...
require 'MEDIA_STREAM_MAX_PUSH_FRAMES = 24' CoreMedia.inc 'device raw push bound must match the amortized browser batch'
...
require 'wifiTestState == WifiTestState::TIME_SYNCING' CorePortal.inc 'stream start must block only active Wi-Fi time verification'
